...
Class FormController extends Controller {
    ...
    public function store(Request $request) {
       ...
       $message = FormMessage::create($data);
        ...
        $text="Новое сообщение на сайте kodiprofessional.com";  // Содержимое отправляемого письма
...
// формируем служебный заголовок письма.
        ...
        $header.="From: =?UTF-8?Q?".str_replace("+","_",str_replace("%","=",urlencode('Новое сообщение')))."?= <$login>\r\n";
        $header.="X-Mailer: Test script hosting Ukraine.com.ua \r\n";
        $header.="Reply-To: =?UTF-8?Q?".str_replace("+","_",str_replace("%","=",urlencode('Новое сообщение')))."?= <$login>\r\n";
        ...
        $header.="Subject: =?UTF-8?Q?".str_replace("+","_",str_replace("%","=",urlencode('сообщение')))."?=\r\n";
